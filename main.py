...
<b>🔥 Покращення v2.8:</b>
• Моделі GPT для аналізу скріншотів (<code>gpt-4o-mini</code>), опису профілю (<code>gpt-4.1</code>) та команди <code>/go</code> (<code>gpt-4.1</code>)[...]
...
async def trigger_vision_analysis_callback(callback_query: CallbackQuery, state: FSMContext):
    ...
    if not photo_file_id:
        ...
        try:
            await callback_query.message.edit_caption(caption="Помилка: дані для аналізу втрачено. Спробуйте надіслати скріншот ще раз.", re[...]
    ...
    try:
        ...
        async with MLBBChatGPT(OPENAI_API_KEY) as gpt_analyzer:
            ...
            if analysis_result_json and "error" not in analysis_result_json:
                ...
                error_msg = analysis_result_json.get('error', 'Невідома помилка аналізу.') if analysis_result_json else 'Відповідь від Vision API не отриман[...]
...
async def main() -> None:
    ...
    try:
        ...
        if bot.session:  # Check if session object exists
            if hasattr(bot.session, "close"):  # Check if it has a close method
                try:
                    await bot.session.close()  # Attempt to close the session
                    # The AiohttpSession.close() method itself handles cases where session is None or already closed.
                    logger.info("Спроба закриття сесії HTTP клієнта Bot виконана.")
                except Exception as e:
                    # Log any error during the explicit close attempt
                    logger.error(f"Помилка під час явного закриття сесії HTTP клієнта Bot: {e}", exc_info=True)
            else:
                # This case should ideally not happen if bot.session is a valid AiohttpSession
