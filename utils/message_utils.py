...
# === ВИРІШЕННЯ ПРОБЛЕМИ: TypeError: unexpected keyword argument 'bot' ===
# Ми оновлюємо сигнатуру функції, додаючи параметр `bot: Bot`.
# Тепер функція офіційно приймає об'єкт бота, що відповідає її виклику
# в `general_handlers.py` і робить її логічно завершеною.
async def send_message_in_chunks(
    ...
    **kwargs: Any
    ...
    - Якщо передано `initial_message_to_edit`, вона редагує це повідомлення
    ...
    Args:
        bot: Екземпляр `Bot` для виконання запитів до API.
    ...
    while len(text) > 0:
        ...
        # Намагаємось розбити по реченню або пробілу для кращої читабельності
    ...
    try:
        ...
        logger.info(f"Надіслано/відредаговано першу частину повідомлення для chat_id {chat_id}.")

        for chunk in chunks:
            await asyncio.sleep(0.5) # Невелика затримка, щоб уникнути флуду
            await bot.send_message(chat_id=chat_id, text=chunk, parse_mode=parse_mode, **kwargs)
            logger.info(f"Надіслано наступну частину повідомлення для chat_id {chat_id}.")
