...
async def update_lobby_message(bot: Bot, chat_id: int):
    ...
    try:
        ...
        # Клавіатура оновлюється динамічно для кожного користувача при взаємодії,
        # але для загального оновлення ми можемо тимчасово не передавати user_id
        # або передавати ID бота, щоб показати "нейтральний" вигляд.
        await bot.edit_message_text(
            text=new_text, chat_id=chat_id, message_id=lobby_data["message_id"],
            reply_markup=create_dynamic_lobby_keyboard(0, lobby_data), # 0 - умовний ID для загального вигляду
...
async def on_party_leave(callback: types.CallbackQuery, bot: Bot):
    user, chat = callback.from_user, callback.message.chat
    lobby = database.get_lobby(chat.id)
    if not lobby or str(user.id) not in lobby["players"] or user.id == lobby["leader_id"]:
        await callback.answer("Ви не в цьому паті або ви є лідером (лідер може тільки скасувати лобі).", show_alert=True)
        return
...
async def cmd_start(message: types.Message, state: FSMContext):
    ...
    welcome_text = f"""Привіт, <b>{user_name}</b>! Я твій AI-помічник для всього, що стосується світу Mobile Legends.\n\n<b>Що я можу:</b>\n• <code>/go &lt;питання&gt;</code> - відповісти на питання по грі.\n• <code>/analyzeprofile</code> - проаналізувати скріншот профілю.\n• <code>/analyzestats</code> - проаналізувати скріншот статистики.\n• Створювати гнучкі лобі для пошуку команди."""
    try:
...
def register_general_handlers(dp: Dispatcher):
    ...
    logger.info("✅ Загальні обробники (v3.0 - Stable) успішно зареєстровано.")
