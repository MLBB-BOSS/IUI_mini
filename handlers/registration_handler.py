...
def format_profile_display(user_data: Dict[str, Any]) -> str:
    ...
    return (
        ...
        f"📊 <b>WR:</b> {wr_str}\n"
        f"🦸 <b>Улюблені герої:</b> {html.escape(heroes)}"
    )
...
async def build_profile_pages(user_data: Dict[str, Any]) -> List[Dict[str, str]]:
    ...
    pages.append({"photo": user_data.get("basic_profile_permanent_url", ""), "caption": caption})
    # stats
...
async def show_profile_menu(
    ...
    """Початкове повідомлення профілю з однією кнопкою."""
