...
PROCESSING_MESSAGES = [
    ...
    "📊 Аналізую цифри та показники...",
    "🧠 Глибокий аналіз даних...",
    "✨ Збираю інсайти для вас...",
    "⚡ Майже готово, останні штрихи...",
    "💡 Генерую персоналізований звіт..."
]
...
async def handle_profile_screenshot(message: Message, state: FSMContext, bot: Bot) -> None:
    ...
    try:
        ...
        await state.update_data(bot_message_id_for_analysis=sent_message.message_id) # Зберігаємо ID повідомлення бота
...
async def trigger_vision_analysis_callback(callback_query: CallbackQuery, state: FSMContext, bot: Bot) -> None:
    ...
    # message_id_with_photo тепер є cq_msg.message_id
    ...
    # bot_message_id_for_analysis - це ID повідомлення, яке бот надіслав з фото та кнопками, тобто cq_msg.message_id
    # Ми будемо редагувати саме cq_msg
    ...
    if not all([photo_file_id, vision_prompt, analysis_type]):
        ...
        try:
            ...
            target_message_to_edit = cq_msg # Повідомлення, яке будемо редагувати
            if target_message_to_edit.caption is not None: # Редагуємо підпис, якщо він є
                 await bot.edit_message_caption(chat_id=chat_id, message_id=target_message_to_edit.message_id, caption=error_text)
            # Якщо немає підпису, але є фото, можна також спробувати edit_message_caption
            elif target_message_to_edit.photo:
                 await bot.edit_message_caption(chat_id=chat_id, message_id=target_message_to_edit.message_id, caption=error_text)
            else: # Якщо це не повідомлення з фото, надсилаємо нове
    ...
    # --- Початкове оновлення повідомлення на "Обробляю..." ---
    try:
        initial_processing_text = f"⏳ {random.choice(PROCESSING_MESSAGES)} {user_name_escaped}..."
        # Редагуємо саме cq_msg (повідомлення з фото та кнопками)
        ...
        await callback_query.answer("Розпочато аналіз...") # Відповідь на колбек
    except TelegramAPIError as e:
        logger.warning(f"Не вдалося відредагувати повідомлення на 'Обробляю...' для {user_name_original} (ID: {callback_query.from_user.id}): {e}")
        # Якщо не вдалося відредагувати, принаймні відповімо на колбек, якщо ще не відповіли
        try: await callback_query.answer("Розпочато аналіз...")
        except TelegramAPIError: pass # Якщо вже відповіли, це викличе помилку, ігноруємо
    ...
    try:
        # --- Етап 1: Завантаження та підготовка зображення ---
        await bot.edit_message_caption(chat_id, cq_msg.message_id, caption=f"🖼️ Завантажую скріншот, {user_name_escaped}...")
        ...
        async with MLBBChatGPT(OPENAI_API_KEY) as gpt_analyzer:
            # --- Етап 2: Аналіз зображення Vision API ---
            await bot.edit_message_caption(chat_id, cq_msg.message_id, caption=f"🤖 Відправляю на аналіз до Vision AI, {user_name_escaped}...")
            ...
            if analysis_result_json and "error" not in analysis_result_json:
                ...
                await bot.edit_message_caption(chat_id, cq_msg.message_id, caption=f"📊 Обробляю результати аналізу, {user_name_escaped}...")

                if analysis_type == "profile":
                    structured_data_text = format_profile_result(user_name_original, analysis_result_json)
                    await bot.edit_message_caption(chat_id, cq_msg.message_id, caption=f"✍️ Генерую опис профілю, {user_name_escaped}...")
                    ...
                    await bot.edit_message_caption(chat_id, cq_msg.message_id, caption=f"📈 Розраховую унікальну статистику, {user_name_escaped}...")
                    ...
                    await bot.edit_message_caption(chat_id, cq_msg.message_id, caption=f"🎙️ Створюю коментар від IUI, {user_name_escaped}...")
                    ...
                    if unique_analytics_formatted and "недостатньо даних" not in unique_analytics_formatted.lower() and "не вдалося розрахувати" not in unique_analy[...]
    ...
    except TelegramAPIError as e: # Ця помилка може виникнути під час edit_message_caption всередині try
        logger.exception(f"Telegram API помилка під час обробки або оновлення повідомлення для {user_name_original}: {e}")
        # Якщо помилка сталася при редагуванні, full_analysis_text_parts може бути порожнім.
        # Встановлюємо текст помилки, якщо ще не встановлено.
        ...
        if not full_analysis_text_parts: # Якщо список порожній, додаємо помилку за замовчуванням
             full_analysis_text_parts.append(default_error_text)

    # --- ФІНАЛЬНЕ ОНОВЛЕННЯ ПОВІДОМЛЕННЯ АБО НАДСИЛАННЯ НОВОГО ---
    final_text_to_send = "\n".join(filter(None, full_analysis_text_parts)).strip()
    if not final_text_to_send: 
        final_text_to_send = default_error_text # Гарантуємо, що текст для надсилання не порожній

    try:
        # cq_msg - це повідомлення, яке бот надіслав з фото та кнопками. Ми його редагуємо.
        # Перевіряємо, чи воно все ще існує і є фото
        if cq_msg and cq_msg.photo: 
            if len(final_text_to_send) <= MAX_TELEGRAM_MESSAGE_LENGTH:
                await bot.edit_message_caption(
                    chat_id=chat_id, message_id=cq_msg.message_id,
            ...
            else: # Підпис задовгий, очищуємо підпис і надсилаємо текст окремо
                logger.warning(f"Підпис до фото ({analysis_type}) для {user_name_original} задовгий. Очищую підпис фото і надсилаю текст окремо.")
                try: 
                    await bot.edit_message_caption(chat_id=chat_id, message_id=cq_msg.message_id, caption="Аналіз завершено. Деталі нижче 👇") 
                except TelegramAPIError: pass # Ігноруємо, якщо не вдалося змінити підпис перед надсиланням нового
                await send_message_in_chunks(bot, chat_id, final_text_to_send, ParseMode.HTML)
        else: # Якщо cq_msg вже не фото або не існує (малоймовірно, але для безпеки)
            logger.info(f"Повідомлення (ID: {cq_msg.message_id if cq_msg else 'N/A'}), яке мало бути фото, не знайдено або не є фото. Надсилаю результат ({analysis_type}) для {user_name_original} окремим повідомленням.")
            await send_message_in_chunks(bot, chat_id, final_text_to_send, ParseMode.HTML)
...
async def cancel_analysis(message: Message, state: FSMContext, bot: Bot) -> None:
    ...
    user_data = await state.get_data()
    # bot_message_id_for_analysis - це ID повідомлення, яке бот надіслав з фото та кнопками
    # Його потрібно видалити, якщо користувач скасовує аналіз на цьому етапі
