...
import base64
import html
import logging # Уже імпортовано з config, але для ясності
import re
...
def calculate_derived_stats(stats_data: Dict[str, Any]) -> Dict[str, Union[str, float, int, None]]:
    ...
    # У Vision промпті ці поля визначені як int, але для розрахунків краще float, якщо вони можуть бути дробовими
    ...
    if matches_played is not None and win_rate_percent is not None:
        total_wins = int(round(matches_played * (win_rate_percent / 100.0))) # Округлення для більшої точності
    ...
    if mvp_count is not None and mvp_count > 0 and mvp_loss_count is not None:
        ...
        if mvp_wins > 0 : 
            # Рахуємо частку MVP у перемогах від загальної кількості MVP, що були у перемогах
            derived['mvp_win_share_percent'] = round((mvp_wins / mvp_count) * 100, 2) if mvp_count > 0 else 0.0
        elif mvp_wins == 0 and mvp_count > 0: # Всі MVP були при поразках
             derived['mvp_win_share_percent'] = 0.0
        else: # Аномальні дані або mvp_wins < 0 (що нелогічно)
...
async def cmd_analyze_profile(message: Message, state: FSMContext) -> None:
    ...
    user_name_escaped = html.escape(user.first_name) # Екрануємо для безпечного використання в HTML
    user_id = user.id
    ...
    await state.update_data(
        ...
        original_user_name=user.first_name # Зберігаємо не екрановане для внутрішньої логіки
    )
...
async def handle_profile_screenshot(message: Message, state: FSMContext, bot: Bot) -> None:
    ...
    # Використовуємо original_user_name, потім екрануємо для відображення
...
def format_profile_result(user_name: str, data: Dict[str, Any]) -> str:
    user_name_escaped = html.escape(user_name) # Екрануємо ім'я користувача
    if not data:
...
async def trigger_vision_analysis_callback(callback_query: CallbackQuery, state: FSMContext, bot: Bot) -> None:
    ...
    if not photo_file_id or not vision_prompt or not analysis_type:
        ...
        try:
            ...
            elif callback_query.message: # Якщо немає підпису, але повідомлення існує
                 await bot.send_message(chat_id, error_text) # Надсилаємо нове, бо редагувати нічого
                        ...
                        if len(detailed_stats_caption) <= 1024: 
                            try:
                                await bot.edit_message_caption(
                                    ...
                                    caption=detailed_stats_caption, parse_mode=ParseMode.HTML
                                )
                                logger.info(f"Оновлено підпис до фото (детальна статистика) для {user_name_original}.")
                            except TelegramAPIError as e_caption:
                                logger.error(f"Не вдалося оновити підпис до фото детальної статистики для {user_name_original}: {e_caption}. Надсилаю текст окремо.")
    ...
    # --- ЗАГАЛЬНИЙ БЛОК НАДСИЛАННЯ РЕЗУЛЬТАТІВ (для "profile" або помилок) ---
    try:
        if callback_query.message and callback_query.message.reply_markup:
             try:
                await bot.edit_message_reply_markup(chat_id=chat_id, message_id=message_id_with_photo, reply_markup=None)
        ...
        if callback_query.message and callback_query.message.photo and len(final_text_for_generic_send) <= 1024:
        ...
        elif callback_query.message: # Якщо повідомлення є, але не фото (або фото вже не існує)
            ...
            await send_message_in_chunks(bot, chat_id, final_text_for_generic_send, ParseMode.HTML)
        else: # Якщо callback_query.message взагалі відсутнє
             logger.error(f"Немає callback_query.message для надсилання фінального тексту для {user_name_original}.")
             # Можливо, варто спробувати надіслати на chat_id, якщо він відомий
             if chat_id:
                await send_message_in_chunks(bot, chat_id, final_text_for_generic_send, ParseMode.HTML)
