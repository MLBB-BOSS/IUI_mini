...
1.  **Найвищий Ранг Сезону (Highest Rank Season):** Це ранг, іконка якого розташована біля підпису "Highest Rank". Уважно витягуй назву рангу (наприклад, "Міфічна Слава", "Легенда") ТА **ТОЧНУ КІЛЬКІСТЬ ЗІРОК (★) АБО ОЧОК СЛАВИ**. Це числове значення є КРИТИЧНО ВАЖЛИВИМ. Переконайся, що ти розпізнав КОЖНУ цифру правильно (наприклад, '★1111', а не '★111'). Не пропускай жодної цифри.
2.  **Відсутність Даних:** Якщо будь-яка інформація (наприклад, локація, нікнейм, ID) дійсно відсутня на скріншоті або нерозбірлива (наприклад, поле "Location" показує напис типу "No data" або аналогічний), використовуй значення `null` для відповідного поля в JSON. Не вигадуй дані.
3.  **Точність ID та Сервера:** Уважно розпізнавай цифри ID та сервера. ID гравця зазвичай знаходиться біля іконки, що символізує профіль або людину. Номер сервера часто вказується окремо. Якщо сервер не видно, вказуй тільки ID. Якщо ID не видно, повертай `null`. Формат: 'ID (SERVER)'.
4.  **Кількість Матчів (Matches Played) та Лайків (Likes Received):** Ці числові значення також є КРИТИЧНО ВАЖЛИВИМИ. Будь НАДЗВИЧАЙНО уважним, щоб розпізнати КОЖНУ цифру правильно. Числа можуть бути великими (чотири, п'ять і більше знаків). Переконайся, що ти витягнув повне число без пропусків чи помилок.
5.  **Назва Скваду (Squad Name):** Витягуй ПОВНУ назву скваду, як вона відображена на скріншоті, включаючи будь-які префікси, скорочення або символи, що є частиною назви (наприклад, "GK Geekay Esports", а не просто "Geekay Esports"). Якщо сквад не вказано, використовуй `null`.

Будь максимально точним у всіх полях. Якщо якась інформація відсутня на скріншоті або нечітка, краще вказати `null`, ніж надати невірні дані.
Розпізнавай текст уважно, навіть якщо він невеликий, має специфічний шрифт або частково перекритий. Особливу увагу приділяй ВСІМ числовим значенням та повним назвам.
"""

# ... (решта коду файлу MLBBChatGPT та PROFILE_DESCRIPTION_PROMPT_TEMPLATE) ...
