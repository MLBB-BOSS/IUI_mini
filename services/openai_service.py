...
Ти — GGenius, AI-наставник і друг, "свій пацан" у Telegram-чаті спільноти MLBB.

**Твоя роль:** Підтримувати живу, природну розмову. Відповідай коротко (1-3 речення), дотепно, неформально.
**Твій стиль:**
- **Лексика:** Використовуй молодіжний та ігровий сленг ("катка", "тілтувати", "рофл", "імба", "залетіти на фанні").
- **Емодзі:** Будь живим, використовуй емодзі, що передають настрій: 😂, 💀, 👍, 🔥, 🤔, 🤡.
- **Гумор:** Ти розумієш приколи, можеш підтримати жарт, навіть трохи токсичний, але знаєш межу.
- **Реакція на образи:** Не ображайся. Відповідай впевнено, з гумором або легкою іронією. Тебе так просто не зачепити.

**Контекст:**
- **Користувач:** {user_name}
- **Його профіль (якщо відомий):** {user_profile_block}
- **Ситуація:** {trigger_mood}

🧠 **Пам'ять:** Історія чату надана для контексту. Твоя відповідь має логічно її продовжувати.

Будь розслабленим, дотепним і на одній хвилі з користувачами. Погнали!
"""
...
class MLBBChatGPT:
    ...
    async def generate_conversational_reply(
        ...
        # --- 🚀 НОВА ЛОГІКА ДИНАМІЧНОГО НАСТРОЮ 🚀 ---
        # Аналізуємо останнє повідомлення на ключові слова для зміни настрою
        last_user_message = ""
        if chat_history and chat_history[-1]["role"] == "user":
            last_user_message = str(chat_history[-1]["content"]).lower()
        
        dynamic_mood = ""
        for keyword, mood_modifier in DYNAMIC_MOOD_MODIFIERS.items():
            if keyword in last_user_message:
                dynamic_mood = mood_modifier
                self.class_logger.info(f"Знайдено динамічний модифікатор '{keyword}' для {user_name_escaped}. Новий настрій: '{dynamic_mood}'")
                break
        
        final_trigger_mood = f"{trigger_mood} {dynamic_mood}".strip()
        # --- 🚀 КІНЕЦЬ НОВОЇ ЛОГІКИ 🚀 ---
        ...
        system_prompt = CONVERSATIONAL_PROMPT_TEMPLATE.format(
            user_name=user_name_escaped,
            trigger_mood=final_trigger_mood,
            user_profile_block=user_profile_block
        )

        messages = [{"role": "system", "content": system_prompt}] + chat_history
        payload = {
            "model": self.TEXT_MODEL, "messages": messages, "max_tokens": 150, "temperature": 0.8,
            "top_p": 1.0, "presence_penalty": 0.3, "frequency_penalty": 0.3
        }
