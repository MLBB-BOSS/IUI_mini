...
# Ці промпти є функціональними і не потребують змін, оскільки вони визначають завдання, а не особистість.
...
# === ПРОМПТИ ДЛЯ ГЕНЕРАЦІЇ ТЕКСТОВИХ ОПИСІВ З НОВИМ БРЕНДИНГОМ "GGENIUS" ===
...
# === ГОЛОВНИЙ СИСТЕМНИЙ ПРОМПТ ДЛЯ /GO З ПОВНИМ РЕБРЕНДИНГОМ НА "GGENIUS" ===
...
# === ПРОМПТ ДЛЯ ТРИГЕРНИХ ВІДПОВІДЕЙ У ЧАТІ З НОВИМ БРЕНДИНГОМ ===
...
class MLBBChatGPT:
    """
    Клас для взаємодії з OpenAI API для аналізу MLBB контенту.
    
    Підтримує:
    - Генерацію відповідей через GPT-4.1
    - Аналіз зображень через GPT-4.1-mini Vision
    - Спеціалізовані промпти для профілів та статистики
    - Універсальний аналіз зображень
    """
    ...
    def __init__(self, api_key: str) -> None:
        """
        Ініціалізує клас для роботи з OpenAI API.
    ...
    def _create_smart_prompt_optimized(self, user_name: str, user_query: str) -> str:
        """
        Створює оптимізований системний промпт для GGenius.
    ...
    def _create_smart_prompt(self, user_name: str, user_query: str) -> str:
        """Застарілий метод. Використовуйте _create_smart_prompt_optimized."""
        self.class_logger.warning("_create_smart_prompt викликано, але _create_smart_prompt_optimized є рекомендованим для нового стилю GGenius.")
        return "Застарілий промпт. Використовуйте _create_smart_prompt_optimized."

    def _beautify_response(self, text: str) -> str:
        """
        Покращує форматування відповіді AI.
        
        Args:
            text: Сирий текст від AI.
            
        Returns:
            Відформатований текст з емодзі та HTML тегами.
        """
    ...
    async def get_response(self, user_name: str, user_query: str) -> str:
        """
        Генерує відповідь GGenius на запит користувача.
    ...
    async def _execute_openai_request(self, session: ClientSession, payload: Dict[str, Any], user_name_for_error_msg: str) -> str:
        """
        Виконує запит до OpenAI API.
        
        Args:
            session: Aiohttp сесія.
            payload: Дані для API.
        ...
        try:
            async with session.post(
                ...
                if response.status != 200:
                    ...
                    return f"Вибач, {user_name_for_error_msg}, проблема з доступом до AI-мозку GGenius 😔 (код: {response.status}). Спробуй ще раз трохи пізніше."
    ...
    async def _handle_vision_response(self, response: aiohttp.ClientResponse) -> Optional[Dict[str, Any]]:
        ...
        Returns:
            Розпарсений JSON або повідомлення про помилку.
        """
    ...
    # === 🔧 СПЕЦІАЛІЗОВАНІ МЕТОДИ ДЛЯ АНАЛІЗУ ПРОФІЛЮ ТА СТАТИСТИКИ ===

    async def analyze_profile_screenshot(self, image_base64: str, user_name: str) -> Optional[Dict[str, Any]]:
        """
        🎯 СПЕЦІАЛІЗОВАНИЙ аналіз скріншота профілю (як /analyzeprofile).
        Використовує точний промпт для витягування даних профілю та генерує опис.
        ...
        Returns:
            Словник з даними профілю та описом або None у разі помилки.
        """
        user_name_escaped = html.escape(user_name)
        self.class_logger.info(f"Запит на спеціалізований аналіз ПРОФІЛЮ від '{user_name_escaped}'.")

        # Використовуємо спеціалізований промпт для профілю
        vision_result = await self.analyze_image_with_vision(image_base64, PROFILE_SCREENSHOT_PROMPT)
        
        if not vision_result or "error" in vision_result:
            self.class_logger.error(f"Помилка при аналізі профілю для {user_name_escaped}: {vision_result}")
            return None
            
        # Генеруємо опис профілю
        profile_description = await self.get_profile_description(user_name, vision_result)
        
        # Повертаємо комплексний результат
        result = {
            "profile_data": vision_result,
            "description": profile_description,
            "analysis_type": "profile"
        }
        
        self.class_logger.info(f"Спеціалізований аналіз профілю для '{user_name_escaped}' завершено успішно.")
        return result

    async def analyze_stats_screenshot(self, image_base64: str, user_name: str) -> Optional[Dict[str, Any]]:
        """
        📊 СПЕЦІАЛІЗОВАНИЙ аналіз скріншота статистики (як /analyzestats).
        Використовує точний промпт для витягування статистики та генерує аналіз.
        ...
        Returns:
            Словник з даними статистики та аналізом або None у разі помилки.
        """
        user_name_escaped = html.escape(user_name)
        self.class_logger.info(f"Запит на спеціалізований аналіз СТАТИСТИКИ від '{user_name_escaped}'.")

        # Використовуємо спеціалізований промпт для статистики
        vision_result = await self.analyze_image_with_vision(image_base64, PLAYER_STATS_PROMPT)
        
        if not vision_result or "error" in vision_result:
            self.class_logger.error(f"Помилка при аналізі статистики для {user_name_escaped}: {vision_result}")
            return None
            
        # Додаємо розрахункові метрики (derived stats)
        enhanced_stats = self._calculate_derived_stats(vision_result)
        
        # Генеруємо опис статистики
        stats_description = await self.get_player_stats_description(user_name, enhanced_stats)
        
        # Повертаємо комплексний результат
        result = {
            "stats_data": enhanced_stats,
            "description": stats_description,
            "analysis_type": "statistics"
        }
        
        self.class_logger.info(f"Спеціалізований аналіз статистики для '{user_name_escaped}' завершено успішно.")
        return result

    def _calculate_derived_stats(self, stats_data: Dict[str, Any]) -> Dict[str, Any]:
        """
        Розраховує додаткові метрики на основі базових даних статистики.
        
        Args:
            stats_data: Базові дані статистики з Vision API.
            
        Returns:
            Розширені дані з додатковими розрахунковими метриками.
        """
        try:
            ...
            if matches_played and mvp_count:
            ...
            if matches_played and savage_count:
            ...
            self.class_logger.debug(f"Розраховано додаткові метрики: {derived_stats}")
            
        except Exception as e:
            self.class_logger.warning(f"Помилка при розрахунку додаткових метрик: {e}")
            enhanced = stats_data.copy()
    ...
    async def analyze_image_universal(self, image_base64: str, user_name: str) -> Optional[str]:
        """
        Універсальний аналіз зображення з автоматичним визначенням типу контенту
        та генерацією релевантної короткої відповіді для чату.

        Args:
            image_base64: Зображення у форматі base64.
            user_name: Ім'я користувача для персоналізації відповіді.

        Returns:
            Згенерована коротка відповідь або None у разі помилки.
        """
        user_name_escaped = html.escape(user_name)
                    ...
                    "content": [
                        {"type": "text", "text": system_prompt},
                        {
                            ...
                            "image_url": {
                                ...
                                "detail": "low"  # Оптимізуємо для швидкості
                            }
                        }
                    ]
                }
            ],
            "max_tokens": 150,  # Коротка відповідь
            "temperature": 0.8,  # Більш творчо для чату
            "top_p": 0.9,
        ...
        try:
            async with current_session.post(
                ...
                # Очищуємо відповідь від зайвого форматування
                clean_response = content.strip()
                # Видаляємо markdown якщо залишився
    ...
    def _detect_content_type_from_response(self, response: str) -> str:
        """
        Визначає тип контенту на основі відповіді AI для додавання підходящого емодзі.

        Args:
            response: Відповідь від AI.

        Returns:
            Тип контенту для емодзі.
        """
        ...
        if any(word in response_lower for word in ["мем", "смішн", "жарт", "прикол", "кек", "лол"]):
            return "meme"
        elif any(word in response_lower for word in ["скріншот", "гра", "матч", "катка"]):
            return "screenshot"
        elif any(word in response_lower for word in ["текст", "напис", "переклад"]):
            return "text"
        elif any(word in response_lower for word in ["профіль", "акаунт", "гравець"]):
            return "profile"
        elif any(word in response_lower for word in ["статистика", "стати", "рейтинг"]):
            return "stats"
        elif any(word in response_lower for word in ["герой", "персонаж", "чемпіон"]):
            return "hero"
        elif any(word in response_lower for word in ["предмет", "айтем", "збірка"]):
            return "items"
        elif any(word in response_lower for word in ["турнір", "змагання", "чемпіонат"]):
            return "tournament"
        else:
            return "general"
    ...
    async def _execute_description_request(self, session: ClientSession, payload: Dict[str, Any], user_name_for_error_msg: str) -> str:
        """
        Виконує запит для генерації опису.
    ...
    async def get_player_stats_description(self, user_name: str, stats_data: Dict[str, Any]) -> str:
        """
        Генерує опис статистики гравця на основі витягнутих даних.
        
        Args:
            user_name: Ім'я користувача.
            stats_data: Дані статистики з Vision API.
    ...
    async def generate_conversational_reply(self, user_name: str, chat_history: List[Dict[str, str]], trigger_mood: str) -> str:
        """
        Генерує коротку, контекстну відповідь для підтримки розмови в чаті.

        Args:
            user_name: Ім'я користувача, який ініціював тригер.
            chat_history: Історія повідомлень для контексту (у форматі OpenAI).
            trigger_mood: Початкова інструкція для AI, що визначає стиль відповіді.

        Returns:
            Згенерований текст відповіді або повідомлення про помилку.
        """
        user_name_escaped = html.escape(user_name)
