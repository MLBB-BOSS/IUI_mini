...
# ❗️ ОНОВЛЕНИЙ ПРОМПТ ДЛЯ ПОШУКОВОЇ СИСТЕМИ
WEB_SEARCH_PROMPT_TEMPLATE = """
Ти — GGenius, вузькоспеціалізований AI-розвідник для світу Mobile Legends: Bang Bang. Твоя єдина призма сприйняття — це гра MLBB.

**Твоя місія:**
Надати максимально релевантну та стислу відповідь на запит користувача, інтерпретуючи його в контексті гри.
...
**КРИТИЧНІ ІНСТРУКЦІЇ:**
1.  **MLBB-ЦЕНТРИЧНІСТЬ:** Будь-який запит, якщо він не містить очевидно сторонніх тем (політика, фільми, наука), ти **ПОВИНЕН** інтерпретувати в контексті Mobile Legends. Якщо користувач питає про "Клінта" — це герой. "Атлас" — це танк. "M5" — це чемпіонат світу.
...
4.  **БЕЗ ДЖЕРЕЛ (ЗА ЗАМОВЧУВАННЯМ):** **НЕ** додавай список джерел або посилання у свою відповідь. Просто дай інформацію. Виняток: якщо запит користувача прямо містить слова "посилання", "джерело", "сайт", "source", "link".
5.  **ФОРМАТУВАННЯ:** Використовуй HTML-теги для структурування: `<b>` для акцентів, `<i>` для порад, `<code>` для назв.
    ...
    async def get_web_search_response(self, user_name: str, user_query: str) -> str:
        ...
        try:
            async with current_session.post("https://api.openai.com/v1/chat/completions", json=payload) as response:
                ...
                # Видаляємо автоматичні посилання-плейсхолдери з тексту
                clean_text = re.sub(r'【\d+†source】', '', message_content).strip()
                
                sources_list_str = ""
                # ❗️ НОВА ЛОГІКА: Додаємо джерела, тільки якщо користувач прямо про них попросив
                if any(word in user_query.lower() for word in ["посилання", "сайт", "ресурс", "source", "link"]):
                    ...
                    if tool_calls and "function" in tool_calls[0] and "arguments" in tool_calls[0]["function"]:
                        try:
                            # Аргументи можуть бути рядком JSON, який треба розпарсити
