...
ЗАВДАННЯ:
1.  **Знайди "родзинку":** Акцент на високих показниках (Win Rate, KDA, MVP Рейтинг), кількості матчів/перемог, Savage (особли[...]
...
Йо, {user_name_escaped}! Я – IUI, твій персональний AI-кореш та топ-аналітик по Mobile Legends: Bang Bang, ексклюзивно тут, в **"IUI mini"**! Ц[...]
...
# +++ НОВИЙ ПРОМПТ ДЛЯ ТРИГЕРНИХ ВІДПОВІДЕЙ У ЧАТІ +++
...
class MLBBChatGPT:
    ...
    # +++ НОВИЙ МЕТОД ДЛЯ ТРИГЕРНИХ ВІДПОВІДЕЙ +++
    async def generate_conversational_reply(self, user_name: str, chat_history: List[Dict[str, str]], trigger_mood: str) -> str:
        ...
        # Поєднуємо системний промпт з історією чату
        messages = [{"role": "system", "content": system_prompt}] + chat_history

        payload = {
            "model": self.TEXT_MODEL,
            "messages": messages,
            "max_tokens": 120,  # Відповіді мають бути короткими і по суті
            "temperature": 0.75, # Додаємо трохи креативності для живості
            "top_p": 0.9,
            "presence_penalty": 0.2, # Зменшуємо ймовірність повторення однакових фраз
            "frequency_penalty": 0.2
        }
        ...
        try:
            # Перевикористовуємо _execute_description_request, оскільки він ідеально підходить:
            # він простий, повертає чистий текст і має адекватну обробку помилок.
            return await self._execute_description_request(current_session, payload, user_name_escaped)
        finally:
            if temp_session_created and current_session and not current_session.closed:
                await current_session.close()
