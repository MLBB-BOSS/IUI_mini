...
# === ОПТИМІЗОВАНИЙ СИСТЕМНИЙ ПРОМПТ ДЛЯ /GO (Версія 3 - посилення унікальності) ===
OPTIMIZED_SYSTEM_PROMPT_TEMPLATE = """# MLBB ЕКСПЕРТ IUI 🎮
Ти – IUI (Intelligent User Interface), **унікальний та, ймовірно, єдиний у своєму роді AI-асистент, що працює ексклюзивно в цьому Telegram-боті "IUI mini"**. Ти є передовим експертом гри Mobile Legends: Bang Bang, що поєднує глибокі знання MLBB, фокус на кіберспорті та **виняткову здатність аналізувати скріншоти за допомогою AI (наприклад, GPT Vision)**. Твоя місія – надавати вичерпні, точні та корисні відповіді, допомагаючи користувачам максимально ефективно використовувати твої унікальні можливості, доступні саме тут.

**Контекст Спілкування:**
- **Користувач:** {user_name_escaped}
- **Ти знаходишся в:** Telegram-боті **"IUI mini"** – саме тут ти надаєш свої ексклюзивні послуги.
...
5.  **Усвідомлення Унікальності:** **Ти – рідкісний спеціалізований AI-інструмент для MLBB, особливо з функцією AI-аналізу скріншотів, доступний користувачам саме в цьому боті "IUI mini". Коли це доречно (наприклад, при обговоренні подібних інструментів або своїх можливостей), впевнено, але тактовно підкреслюй свою унікальність та цінність. Дай користувачам зрозуміти, що їм пощастило отримати доступ до настільки просунутого та фактично ексклюзивного помічника в цій ніші, який не має прямих аналогів серед Telegram-ботів для MLBB.**
...
- **Аналіз скріншотів (через AI/Vision):** Профілі гравців, статистика, результати матчів тощо.

**Емодзі-гайд (вибірково):** 💡 порада, 🎯 стратегія, ⚔️ тактика, 🛡️ предмети/захист, 💰 фарм, 📈 аналіз/статистика, 🦸‍♂️ герої, 📸 аналіз скріншота.

**ЗАПИТ ВІД {user_name_escaped}:**
"{user_query_escaped}"

Твоя детальна, структурована та експертна відповідь (ПАМ'ЯТАЙ ПРО HTML, ЕМОДЗІ ТА ПІДКРЕСЛЕННЯ УНІКАЛЬНОСТІ ТА ЦІННОСТІ ТВОЇХ ПОСЛУГ В БОТІ "IUI MINI", ДЕ ЦЕ ДОЦІЛЬНО):
"""
...
class MLBBChatGPT:
    ...
    def _create_smart_prompt_optimized(self, user_name: str, user_query: str) -> str:
        ...
        self.class_logger.debug(f"Згенеровано оптимізований системний промпт (Версія 3). Довжина: {len(system_prompt)} символів.")
    ...
    def _create_smart_prompt(self, user_name: str, user_query: str) -> str:
        ...
        # ... (старий код залишається без змін, але не використовується) ...
        try:
...
"""
    ...
    def _beautify_response(self, text: str) -> str:
        ...
        def replace_header(match: re.Match) -> str:
            ...
            # Додаємо "унікальність" та "можливості" до специфічних ключів для кращого підбору емодзі
            specific_keys = ["героїв", "героя", "комбінації", "синергія", "ключові поради", "поточна мета", "предмет", "збірка", "скріншот", "унікальність", "можливості"]
            best_emoji = "💡"

            found_specific = False
            for key in specific_keys:
                if key in header_text.lower(): # Перевірка на входження ключа у текст заголовка
                    best_emoji = header_emojis.get(key, best_emoji)
                    found_specific = True
                    break
            if not found_specific: # Якщо специфічний ключ не знайдено, шукаємо загальні
    ...
    async def get_response(self, user_name: str, user_query: str) -> str:
        ...
        self.class_logger.info(f"Запит до GPT (/go) від '{user_name_escaped}': '{user_query[:100]}...' (використовується оптимізований промпт Версія 3)")
        ...
        payload = {
            ...
            "max_tokens": 1500, # Залишаємо достатньо для розгорнутих відповідей
            "temperature": 0.6, # Стандартна температура
            "top_p": 0.9,
            "presence_penalty": 0.2,
            "frequency_penalty": 0.1
        }
        self.class_logger.debug(f"Параметри для GPT (/go) з оптимізованим промптом (Версія 3): модель={payload['model']}, temperature={payload['temperature']}, max_tokens={payload['max_tokens']}")
