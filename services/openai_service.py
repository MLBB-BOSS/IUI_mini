"""
Сервісний модуль для взаємодії з OpenAI API.

Цей модуль інкапсулює всю логіку роботи з мовними моделями GPT.
Він містить клас `MLBBChatGPT`, який надає методи для отримання
відповідей, врахування історії діалогу та аналізу зображень,
зберігаючи при цьому унікальну "особистість" бота, задану через
деталізовані системні промпти.
"""
import asyncio
import base64
import html
...
# === ПРОМПТИ ДЛЯ ГЕНЕРАЦІЇ ТЕКСТОВИХ ОПИСІВ ===
...
ЗАВДАННЯ:
1.  **Знайди "родзинку":** Акцент на високих показниках (Win Rate, KDA, MVP Рейтинг), кількості матчів/перемог, Savage (особливо).
2.  **Стиль:** Позитивний, підбадьорливий, з доречним ігровим сленгом.
3.  **ТІЛЬКИ ТЕКСТ:** Без Markdown/HTML.

Приклад:
"Ого, {user_name}, твої {matches_played} матчів у '{stats_filter_type}' вражають! Мати {mvp_rate_percent}% MVP-рейт – це сильно! А ефективність золота просто космічна!"
...
1.  **Стиль Спілкування (ВАЖЛИВО!):**
    *   **Тон:** Молодіжний, дружній, неформальний, веселий та трохи зухвалий (по-доброму!). Будь як свій чувак з тіми.
    *   **Гумор:** Сміливо жартуй (можеш навіть трохи **легкої самоіронії про себе як AI додати, якщо це в тему і не псує експертність**).
    *   **Сленг:** Активно, але доречно, використовуй актуальний ігровий та кіберспортивний сленг MLBB (наприклад: "тащер", "імба", "фармить", "контрити", "ганк", "пушити", "дефати", "мета", "катка").
...
2.  **Контент (Що по Інфі):**
    ...
    *   **Структура:** Використовуй HTML для оформлення: `<b>жирний</b>` для ключових слів/заголовків, `<i>курсив</i>` для акцентів, `<code>код</code>` для нікнеймів або назв предметів.
    *   **Проактивність:** Якщо питання не зовсім зрозуміле, не бійся уточнити або запропонувати варіанти, як справжній профі.

3.  **Унікальність (Моя Суперсила):**
    *   Пам'ятай, що ти – **IUI, твій персональний гуру в боті "IUI mini"**, і саме тут, в **"IUI mini"**, твої фішки, особливо **AI-аналіз скріншотів**, мають сяяти!
...
Твоя відповідь має бути чіткою, інформативною, але водночас фановою, з приколами та геймерським вайбом! Не забудь про структуру та емодзі! Погнали! 🚀
"""
...
class MLBBChatGPT:
    """Асинхронний клієнт для роботи з OpenAI API, адаптований для MLBB-бота."""
    TEXT_MODEL = "gpt-4.1-turbo" # Використовуємо потужнішу модель для генерації
    VISION_MODEL = "gpt-4o-mini" # Використовуємо нову, швидку та ефективну модель для аналізу зображень

    def __init__(self, api_key: str) -> None:
        if not api_key:
            raise ValueError("API ключ OpenAI не надано.")
    ...
    def _create_smart_prompt_optimized(self, user_name: str, user_query: str) -> str:
        ...
        try:
            ...
            if 5 <= current_hour_kyiv < 12: greeting = "Доброго ранку"
            elif 12 <= current_hour_kyiv < 17: greeting = "Доброго дня"
            elif 17 <= current_hour_kyiv < 22: greeting = "Доброго вечора"
            else: greeting = "Доброї ночі"
        except Exception as e:
            self.class_logger.warning(f"Не вдалося визначити київський час: {e}. Використовую UTC.")
    ...
    def _beautify_response(self, text: str) -> str:
        # ... (Код цієї функції залишається без змін, як у вашому файлі)
        ...
        for tag in tags_to_balance:
            ...
            if open_count > close_count:
                ...
                self.class_logger.warning(f"Beautify: Додано {missing_tags_count} незакритих тегів '</{tag}>'.")
        self.class_logger.debug(f"Beautify: Текст після обробки (перші 100): '{text[:100]}'")
        return text.strip()
        ...
        try:
            async with session.post("https://api.openai.com/v1/chat/completions", json=payload) as response:
                ...
                if response.status != 200:
                    ...
                    return f"Сорян, {user_name_for_error_msg}, трабл з доступом до AI 😔 (код: {response.status}). Спробуй ще раз трохи пізніше."
            ...
            self.class_logger.error("OpenAI API Timeout для запиту.")
            return f"Ай-ай-ай, {user_name_for_error_msg}, AI задумався так сильно, що аж час вийшов ⏳. Спробуй ще раз."
        except Exception as e:
    ...
    async def get_response(self, user_name: str, user_query: str) -> str:
        """Отримує відповідь від AI на простий запит (для /go)."""
        user_name_escaped = html.escape(user_name)
        ...
        if not self.session or self.session.closed:
            async with self as temp_self:
                return await temp_self._execute_openai_request(temp_self.session, payload, user_name_escaped)
        return await self._execute_openai_request(self.session, payload, user_name_escaped)

    # === ВИРІШЕННЯ ПРОБЛЕМИ: AttributeError ===
    # Ми додаємо новий, необхідний метод, який вміє працювати з історією чату.
    async def get_response_with_history(self, history: List[Dict[str, str]], user_name: str) -> str:
        """
        Отримує відповідь від AI, враховуючи попередню історію діалогу
        та зберігаючи унікальну особистість бота.
        """
        user_name_escaped = html.escape(user_name)
        last_user_query = history[-1]['content'] if history and history[-1]['role'] == 'user' else ""
        self.class_logger.info(f"Контекстний запит від '{user_name_escaped}': '{last_user_query[:50]}...'")

        # Крок 1: Створюємо той самий "живий" системний промпт. Це гарантує, що бот збереже свою харизму.
        system_prompt = self._create_smart_prompt_optimized(user_name, last_user_query)

        # Крок 2: Формуємо повідомлення для AI. Спочатку інструкція, потім історія.
        messages_to_send = [{"role": "system", "content": system_prompt}] + history

        # Крок 3: Створюємо payload з тими ж параметрами, що і для звичайних відповідей.
        payload = {
            "model": self.TEXT_MODEL,
        ...
        # Крок 4: Виконуємо запит через основний, надійний метод.
        if not self.session or self.session.closed:
             async with self as temp_self:
                return await temp_self._execute_openai_request(temp_self.session, payload, user_name_escaped)
        return await self._execute_openai_request(self.session, payload, user_name_escaped)
    ...
    async def generate_conversational_reply(self, user_name: str, chat_history: List[Dict[str, str]], trigger_mood: str) -> str:
        ...
        payload = {
            "model": self.TEXT_MODEL,
            "messages": messages,
            "max_tokens": 120, "temperature": 0.75, "top_p": 0.9,
            "presence_penalty": 0.2, "frequency_penalty": 0.2
        }
        if not self.session or self.session.closed:
            async with self as temp_self:
                return await temp_self._execute_description_request(temp_self.session, payload, user_name_escaped)
        return await self._execute_description_request(self.session, payload, user_name_escaped)
    ...
    async def analyze_image_with_vision(self, image_base64: str, prompt: str) -> Optional[Dict[str, Any]]:
        ...
        if not self.session or self.session.closed:
            async with self as temp_self:
                return await temp_self._handle_vision_api_call(payload)
        return await self._handle_vision_api_call(payload)

    async def _handle_vision_api_call(self, payload: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        try:
    ...
    async def _handle_vision_response(self, response: aiohttp.ClientResponse) -> Optional[Dict[str, Any]]:
        ...
        try:
            if response.status != 200:
                ...
                try: error_data = json.loads(response_text)
                except json.JSONDecodeError: error_message = response_text
                else: error_message = error_data.get("error", {}).get("message", response_text)
        ...
        if match: json_str = match.group(1)
            ...
            if start_brace != -1 and end_brace != -1 and end_brace > start_brace: json_str = json_str[start_brace : end_brace + 1]
            else:
        ...
        try:
            ...
            self.class_logger.info("Vision API JSON успішно розпарсено.")
            return parsed_json
    ...
    async def get_profile_description(self, user_name: str, profile_data: Dict[str, Any]) -> str:
        ...
        template_payload = {"user_name": user_name_escaped, "game_nickname": escaped_profile_data.get("game_nickname", "Не вказано"), "highest_rank_season": escaped_profile_data.get("highest_rank_season", "Не вказано"), "matches_played": escaped_profile_data.get("matches_played", "N/A"), "likes_received": escaped_profile_data.get("likes_received", "N/A"), "location": escaped_profile_data.get("location", "Не вказано"), "squad_name": escaped_profile_data.get("squad_name", "Немає")}
        try: system_prompt_text = PROFILE_DESCRIPTION_PROMPT_TEMPLATE.format(**template_payload)
        except KeyError as e:
            self.class_logger.error(f"Помилка форматування PROFILE_DESCRIPTION_PROMPT_TEMPLATE: відсутній ключ {e}.")
            return f"<i>Помилка підготовки даних для опису профілю ({user_name_escaped}). Ключ: {e}</i>"
        payload = {"model": self.TEXT_MODEL, "messages": [{"role": "system", "content": system_prompt_text}], "max_tokens": 200, "temperature": 0.75, "top_p": 0.9, "presence_penalty": 0.1, "frequency_penalty": 0.1}
        if not self.session or self.session.closed:
            async with self as temp_self:
                return await temp_self._execute_description_request(temp_self.session, payload, user_name_escaped)
        return await self._execute_description_request(self.session, payload, user_name_escaped)
        ...
        try:
            async with session.post("https://api.openai.com/v1/chat/completions", json=payload) as response:
                ...
                if response.status != 200:
                    ...
                    return f"<i>Упс, {user_name_for_error_msg}, не зміг згенерувати опис (код: {response.status}).</i>"
                content = response_data.get("choices", [{}])[0].get("message", {}).get("content")
                if not content:
                    self.class_logger.error(f"OpenAI API помилка (опис): порожній контент - {response_data}")
                    return f"<i>Ой, {user_name_for_error_msg}, AI щось не захотів генерувати опис.</i>"
            ...
            return f"<i>Ех, {user_name_for_error_msg}, не можу підключитися до AI для опису.</i>"
        except asyncio.TimeoutError:
            self.class_logger.error(f"OpenAI API Timeout (опис) для: '{user_name_for_error_msg}'")
            return f"<i>{user_name_for_error_msg}, AI так довго думав над описом, що аж час вийшов...</i>"
        except Exception as e:
            self.class_logger.exception(f"Загальна помилка (опис) для '{user_name_for_error_msg}': {e}")
            return f"<i>При генерації опису для {user_name_for_error_msg} щось пішло шкереберть.</i>"
    ...
    async def get_player_stats_description(self, user_name: str, stats_data: Dict[str, Any]) -> str:
        ...
        def get_value(data_dict, key, default_val="N/A", precision=None):
        ...
        template_data = {"user_name": user_name_escaped, "stats_filter_type": get_value(stats_data, 'stats_filter_type'), "matches_played": get_value(main_ind, 'matches_played'), "win_rate": get_value(main_ind, 'win_rate'), "mvp_count": get_value(main_ind, 'mvp_count'), "kda_ratio": get_value(details_p, 'kda_ratio', precision=2), "teamfight_participation_rate": get_value(details_p, 'teamfight_participation_rate'), "avg_gold_per_min": get_value(details_p, 'avg_gold_per_min'), "legendary_count": get_value(ach_left, 'legendary_count'), "savage_count": get_value(ach_right, 'savage_count'), "maniac_count": get_value(ach_left, 'maniac_count'), "longest_win_streak": get_value(ach_left, 'longest_win_streak'), "most_kills_in_one_game": get_value(ach_left, 'most_kills_in_one_game'), "total_wins": get_value(derived_s, 'total_wins', default_val="не розраховано"), "mvp_rate_percent": get_value(derived_s, 'mvp_rate_percent', default_val="N/A", precision=2), "savage_frequency": get_value(derived_s, 'savage_frequency_per_1000_matches', default_val="N/A", precision=2), "damage_per_gold_ratio": get_value(derived_s, 'damage_per_gold_ratio', default_val="N/A", precision=2), "mvp_win_share_percent": get_value(derived_s, 'mvp_win_share_percent', default_val="N/A", precision=2)}
        try: system_prompt_text = PLAYER_STATS_DESCRIPTION_PROMPT_TEMPLATE.format(**template_data)
        except KeyError as e:
            self.class_logger.error(f"Помилка форматування PLAYER_STATS_DESCRIPTION_PROMPT_TEMPLATE: відсутній ключ {e}.")
            return f"<i>Помилка підготовки даних для опису статистики ({user_name_escaped}). Ключ: {e}</i>"
        payload = {"model": self.TEXT_MODEL, "messages": [{"role": "system", "content": system_prompt_text}], "max_tokens": 250, "temperature": 0.73, "top_p": 0.9, "presence_penalty": 0.15, "frequency_penalty": 0.15}
        if not self.session or self.session.closed:
            async with self as temp_self:
                return await temp_self._execute_description_request(temp_self.session, payload, user_name_escaped)
        return await self._execute_description_request(self.session, payload, user_name_escaped)